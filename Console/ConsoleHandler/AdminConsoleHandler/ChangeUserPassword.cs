using Application.Contracts;
using Application.Models;
using Spectre.Console;

namespace Console.ConsoleHandler.AdminConsoleHandler;

public class ChangeUserPassword : IHandler
{
    private readonly IAdminService _adminService;

    public ChangeUserPassword(IAdminService adminService)
    {
        _adminService = adminService;
    }

    public string Name => "Change password";

    public void Handle()
    {
        string name = AnsiConsole.Ask<string>("Enter name");
        int newPassword = AnsiConsole.Ask<int>("Enter new password");

        User? user = _adminService.GetUserByName(name).WaitAsync(CancellationToken.None).Result;
        if (user is null)
        {
            throw new ArgumentException("There no such user");
        }

        _adminService.ChangeUserPassword(user, newPassword);
    }
}